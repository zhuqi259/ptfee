...
<script type="text/javascript">
	...
	$(function() {
				...
				.dataTable(
						{
							"bProcessing" : true,
							"bServerSide" : true,
							"sAjaxSource" : "system/activityPayment!allPayments.action?id="
									+ activityId,
							"sPaginationType" : "full_numbers",
							"oLanguage" : {
								"sProcessing" : "正在处理...",
								"sLengthMenu" : "每页 _MENU_ 行<input type='button' value='删除财务' onclick='deleteZPayment()'><input type='button' value='更新财务' onclick='updateZPayment()'>",
								"sZeroRecords" : "没有找到记录",
								"sEmptyTable" : "没有找到记录",
		...
		$("#zpaymentList tbody").click(function(event) {
	...
	function updateZPayment() {
		...
		//获得被选中记录的ID
		var zpaymentId = anSelected[0].children[0].innerHTML;

		window.location = "<%=basePath%>system/zpayment!updateInput.action?id=" + zpaymentId;
	}

	function deleteZPayment() {
		...
		if (!confirm("请确认是否要删除选中的财务信息！")) {
			return;
		}
		//获得被选中记录的ID
		var zpaymentId = anSelected[0].children[0].innerHTML;
		$.get("<%=basePath%>system/zpayment!del.action?id=" + zpaymentId, function() {
...
<body>
	...
	<div id="formwrapper">
		...
		<form action="system/activity.action" method="post"
			...
			<fieldset>
				<legend>
					流程结点财务信息<input type="button" value="添加财务信息"
						onclick="window.location = '<%=basePath%>system/zpayment!addInput.action?activity.id=${id}'" />
	...
	<!-- 显示activity下面的财务信息 -->
	<div>
		<table id="zpaymentList" class="display" width="100%" border="0"
			...
			<thead>
				<tr>
					...
					<th width="60" height="20" bgcolor="d3eaef" class="STYLE6"><div
							align="center">
							<span class="STYLE10">财务信息名称</span>
						</div></th>
				</tr>
			</thead>
			<tbody>
