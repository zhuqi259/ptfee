...
<div id="formwrapper">
	...
	<fieldset>
		...
		<input type="button" value="添加子公司" onclick="window.location = '<%=basePath%>system/company!addInput.action?parent.id=${id}'"/>
		</s:if>
		<s:if test="permit('company','DELETE')">
		<input type="button" value="删除公司信息" onclick="window.location = '<%=basePath%>system/company!del.action?id=${id}'"/>
		</s:if>
		</legend>
		<div>
